...
The users table contains columns:  
`id`, `username`, and `hash` which prevents the passwords from being stored in plain text.

The spots table contains the columns:  
`id`, `user_id`, `name`, `address`, `rating`, `notes`, `lat`, `lon`, `image_path`, and `data travel`. This table has hot chocolate spot data and is linked with the user who added the data.
...
**Export to KML**: Users have the ability to export locations as .kml files that can be viewed in Google Maps or Google Earth.
